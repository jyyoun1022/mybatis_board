...
public class SignUpRequestValidateTest {
    ...
    void invalidateByShortUsernameTest() {
        ...
        // then
        assertThat(validate).isNotEmpty();
        for (ConstraintViolation<SignUpRequest> signUpRequestConstraintViolation : validate) {
            System.out.println(signUpRequestConstraintViolation);
        }
        assertThat(validate.stream().map(v -> v.getInvalidValue()).collect(toSet())).contains(invalidValue);
    }
    ...
    private SignUpRequest createRequest() {
        return new SignUpRequest("email@email.com", "123456a!", "username", "nickname","1996-10-22");
    }

    private SignUpRequest createRequestWithEmail(String email) {
        return new SignUpRequest(email, "123456a!", "username", "nickname","1996-10-22");
    }

    private SignUpRequest createRequestWithPassword(String password) {
        return new SignUpRequest("email@email.com", password, "username", "nickname","1996-10-22");
    }
